  ...
  useEffect(() => {
    ...
    const prepare = async () => {
      ...
      images.forEach(image => {
        image.loading = 'eager'
        image.closest('[data-image-state]')?.setAttribute('data-entry-image', '')
      })
      const background = new Image()
      background.src = '/assets/stripes.svg'
      await Promise.all([document.fonts.ready, decode(background), ...images.map(decode)])
    ...
    void prepare().catch(() => { if (!cancelled) window.__revealPortfolio?.() })
