...
The preview uses a horizontal navigation bar, a large typographic hero with the original illuminated monogram, slow decorative orbits, and a quieter grid background. The original teal/blue palette and coordinated ring-reflection opening are preserved. Work appears first as a featured project and four large screenshot cards; older projects remain in an expandable archive. The about section pairs the existing portrait with a short introduction and working principles. A two-column contact section keeps the existing Netlify form and social destinations.

Mobile navigation uses an accessible expandable menu. Layouts stack at narrow widths, form inputs use 16px text to avoid focus zoom, keyboard focus stays visible, and decorative motion respects reduced-motion preferences. Existing project URLs, modal scroll restoration, and form field names are preserved.
